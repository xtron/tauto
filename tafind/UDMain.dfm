object DMMain: TDMMain
  ...
  object ClientFindQ: TIB_Query
    ...
    IB_Connection = IBC
    SQL.Strings = (
      'SELECT CLIENTID,LNAME,FNAME.FNAME,MNAME.MNAME FROM CLIENTS '
      'LEFT JOIN FNAME ON FNAME.FNAMEID = CLIENTS.FNAMEID'
      'LEFT JOIN MNAME ON MNAME.MNAMEID = CLIENTS.MNAMEID')
  ...
  object OrderFindQ: TIB_Query
    ...
    SQL.Strings = (
        ...
        'SELECT (C.LNAME || '#39' '#39' || FNAME.FNAME || '#39' '#39' || MNAME.MNAME) FIO' +
        ',O.ORDERID,O.ORDERDATE,O.STATUS,C.CLIENTID FROM ORDERS O'
      'LEFT JOIN CLIENTS C ON C.CLIENTID = O.CLIENTID'
      'LEFT JOIN FNAME ON FNAME.FNAMEID = C.FNAMEID'
      'LEFT JOIN MNAME ON MNAME.MNAMEID = C.MNAMEID')
    Left = 56
    Top = 112
  end
